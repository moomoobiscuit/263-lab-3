# ENGSCI263: Tutorial Lab 3 - Forecasting Geothermal Impacts
# main.py
...
def construct_priors():
	...
	mu_logk = np.mean(np.log10(ks))			
	std_logk = np.std(np.log10(ks))
...
def model_ensemble(N):
	...
	while cnt < N:
		...
		phi = np.random.randn() * std_phi + mu_phi
		...
		logk = np.random.randn() * std_logk + mu_logk
		k = 10**logk
		...
		out = forward_model(k, phi, price_electricity, costs, plot = False, show_me_more=False)
		outs.append(out)
...
def decision(predictions, threshold, xlabel='predictions', thresholdlabel='threshold'):
	...
	Pexceed =  len([np.where(predictions > 0.7)])/N
